...
rgp::FontFactory::FontFactory() : m_fontArray([] {
    ...
    constexpr float LOAD_SIZE = BASE_SIZE * OVERSAMPLE_SCALE;

    std::array<std::unique_ptr<Font>, static_cast<size_t>(FontType::Count)> arr{};

    arr[static_cast<size_t>(FontType::ZenMaruMedium32Left)] =
        std::make_unique<Font>(ZEN_MARU_GOTHIC_MEDIUM, LOAD_SIZE, TTF_HORIZONTAL_ALIGN_LEFT);
    arr[static_cast<size_t>(FontType::ZenMaruMedium32Center)] =
        std::make_unique<Font>(ZEN_MARU_GOTHIC_MEDIUM, LOAD_SIZE, TTF_HORIZONTAL_ALIGN_CENTER);
    arr[static_cast<size_t>(FontType::ZenMaruMedium32Right)] =
        std::make_unique<Font>(ZEN_MARU_GOTHIC_MEDIUM, LOAD_SIZE, TTF_HORIZONTAL_ALIGN_RIGHT);
