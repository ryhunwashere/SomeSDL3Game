...
rgp::PlayerEntity::PlayerEntity(const std::string& texturePath) {
	m_textureInfo = &rgp::TextureEngine::get().loadPNGTexture(texturePath, TEXTURE_SIZE);
}

rgp::PlayerEntity::~PlayerEntity() {
	if (m_textureInfo->texture) {
		SDL_Log("Unloading player texture...");
		rgp::TextureEngine::get().unloadTexture(m_textureInfo->path);
	}
...
void rgp::PlayerEntity::draw() {
	...
	rgp::RendererEngine::get().draw(
		1.0, 1.0, 1.0, SDL_ALPHA_OPAQUE_FLOAT, m_textureInfo->texture, &dstRect
	);
}
